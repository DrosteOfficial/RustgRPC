...
use crate::generated::messages::{CreateMessageRequest, DeleteMessageRequest, GetMessageRequest, Message, MessageResponse};
...
impl MessagesService {
    ...
    async fn delete_message(&self, request: Request<DeleteMessageRequest>) -> Result<Response<MessageResponse>, Status> {
        ...
        let _message_exist = self.get_messages(Request::new(GetMessageRequest {
        ...
        let _deleted_message = messageEntity::Entity::delete_by_id(det.message_id).exec(&_db_conn).await; //message id not user id
